...
class Database:
    ...
    def encode_channel_id(channel_id: int) -> str:
        """Encode channel ID to base64 for deep links"""
        channel_str = str(abs(channel_id))
        encoded = base64.b64encode(channel_str.encode()).decode()
        return encoded
    
    @staticmethod
    def decode_channel_id(encoded: str) -> Optional[int]:
        """Decode base64 to channel ID"""
        try:
            # Remove 'req_' prefix if present
            if encoded.startswith('req_'):
                encoded = encoded[4:]
            
            decoded = base64.b64decode(encoded.encode()).decode()
            return -int(decoded)  # Return as negative (channel IDs are negative)
        except Exception as e:
            logger.error(f"Failed to decode channel ID: {e}")
            return None
